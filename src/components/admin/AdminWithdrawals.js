...
function AdminWithdrawals() {
  ...
  const { userWithdrawals, userSavingWithdrawals } = useSelector(
  ...
  useFirestoreConnect([
    {
      ...
      orderBy: ['date', 'desc'],
      storeAs: 'userWithdrawals',
    },

    {
      collectionGroup: 'savingWithdrawals',

      orderBy: ['date', 'desc'],

      storeAs: 'userSavingWithdrawals',
    },
  ])
  ...
  const deleteSuccessOption = {
    title: <p>Success</p>,
    text: <span className="text-success">Document successfully deleted</span>,
    icon: 'success',
    showCloseButton: true,
  }
  const notFoundError = {
  ...
  const handleSavingWithdrawalUpdate = () => {
    const docRef = firebase
      .firestore()
      .collection('savings')
      .doc(userInfo.uid)
  ...
  const handleSavingWithdrawalDelete = (each) => {
    const docRef = firebase
      .firestore()
      .collection('savings')
      .doc(each.uid)
  ...
  const handleWithdrawalDelete = (each) => {
    const docRef = firebase
      .firestore()
      .collection('withdrawals')
      .doc(each.uid)
              ...
              <div className="card-body tab-content p-0">
                ...
                <div className="tab-pane fade" id="Weekly">
                  <div className="table-responsive">
                    <table className="table shadow-hover card-table border-no tbl-btn short-one">
                      <tbody>
                        {userSavingWithdrawals &&
                          userSavingWithdrawals.map((each) => (
                            <>
                              <tr
                                key={each.date}
                                onClick={() =>
                        ...
                        {userSavingWithdrawals?.length === 0 && (
                          <tr className="text-center text-warning row-span-4">
                            <td colSpan={6}>No Transaction Yet</td>
                          </tr>
                        )}
